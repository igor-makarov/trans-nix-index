...
def write_inputs(work):
    ...
    graph = os.path.join(work, "graph.jsonl")
    ...
    for stem, obj in [
        ...
        with open(os.path.join(prev, f"{stem}.json"), "wt") as f:
            json.dump(obj, f)
...
def read(out_dir, name):
    return json.load(open(os.path.join(out_dir, name), "rt"))
...
def main():
    ...
    with tempfile.TemporaryDirectory() as work:
        ...
        info = read(out_dir, "info-indexed.json")
        refs = read(out_dir, "refs-indexed.json")
        closures = read(out_dir, "closures.json")
