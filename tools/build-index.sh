...
extract_one() {
  ...
  if python3 "$HERE/extract-index.py" \
       "$NIXDIR/extract-versions.nix" "$NIXDIR/nested-sets.nix" \
       "$src" x86_64-linux "$dest.tmp" 2>"$dest.err"; then
    mv "$dest.tmp" "$dest"
...
")
  ...
  if [ "$JOBS" -gt 1 ]; then
    ...
    if ! printf '%s\n' "${TARGETS[@]}" | xargs -r -P "$JOBS" -L 1 bash "$0" --extract-one; then
      FAILURES=1
    fi
  else
