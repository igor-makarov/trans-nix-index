...
Usage: build-site-data.py <repo> <datadir> <out>
  ...
  outs-indexed.json, and the
           graph artifacts either whole (info-indexed.json, ...) or as
           period shards (info-indexed-2024.json, ...) — every file
           matching <stem>*.json is merged
...
class DiskMap:
    """Large reference/output tables live on disk, not in Python's heap."""

    def __init__(self, files):
        self.temporary = tempfile.TemporaryDirectory()
        self.db = sqlite3.connect(J(self.temporary.name, "data.sqlite"))
        self.db.execute("PRAGMA journal_mode=OFF")
        self.db.execute("PRAGMA synchronous=OFF")
        self.db.execute("PRAGMA cache_size=-8192")
        self.db.execute(
            "CREATE TABLE entries (key TEXT PRIMARY KEY, value TEXT) WITHOUT ROWID"
        )
        for path in files:
            values = load(path)
            self.db.executemany(
...
def load_stem(stem):
    """Merge <stem>.json / <stem>-<period>.json — whichever exist."""
    merged = {}
    files = sorted(glob.glob(J(datadir, f"{stem}*.json")))
    if not files:
        print(f"NOTE: no {stem} files, building without them")
        return merged
    if stem in {"refs-indexed", "outs-indexed"}:
...
def build_meta_shards(pairs, by_digest, by_name, meta_dir, revdeps=None):
    ...
    current_key, b, count = None, None, 0
    # Flush each shard instead of holding every system's rendered metadata in RAM.
    for (attr, ver), (digest, name, _is_tip) in sorted(
        pairs.items(), key=lambda item: (shard_key(item[0][0]), item[0])
    ):
        key = shard_key(attr)
        if key != current_key:
            if b is not None:
                dump(
                    {"paths": b.paths, "attrs": b.attrs},
                    J(meta_dir, current_key + ".json"),
                )
            current_key, b = key, MetaShard()
            count += 1
        seen_targets = set()
        inf = info.get(digest)
        ...
        def add_refs(rlist):
            ...
            for base in rlist:
                ...
                if (
                    ...
                    and hit
                    and hit != (attr, ver)
                    and hit not in seen_targets
                ):
                    revdeps[hit[0]][hit[1]].add((attr, ver))
                    seen_targets.add(hit)
            return r_idx
    ...
    if b is not None:
        dump({"paths": b.paths, "attrs": b.attrs}, J(meta_dir, current_key + ".json"))
    return count
...
class CappedDeps:
    """Exact count, but only the lexicographically first REVDEP_CAP examples."""
    ...
    def add(self, pair):
        # build_meta_shards calls once per distinct consumer/target pair.
        self.count += 1
        if len(self.sample) < REVDEP_CAP:
            self.sample.add(pair)
            if len(self.sample) == REVDEP_CAP:
                self.largest = max(self.sample)
        elif pair < self.largest:
            self.sample.remove(self.largest)
            self.sample.add(pair)
            self.largest = max(self.sample)
...
revdeps = defaultdict(lambda: defaultdict(CappedDeps))
...
for system in alt_systems:
    ...
    alt_revdeps = defaultdict(lambda: defaultdict(CappedDeps))
    ...
    del alt_pairs, alt_by_digest, alt_by_name, alt_revdeps
