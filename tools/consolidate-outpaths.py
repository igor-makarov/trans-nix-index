...
"""Consolidate the narinfo crawl graph into three compact files.
  ...
  info-indexed.json     digest -> [ok, narSize, fileSize, name, narUrl]
  refs-indexed.json     digest -> [ref basename, ...]   (direct refs)
  closures.json         digest -> [closureBytes, closurePaths, deadInClosure]
...
period shards — every file matching <stem>*.json is read). A digest the
...
def load_stem(prev_dir, stem):
    """Merge <stem>.json / <stem>-<period>.json from prev_dir."""
    merged = {}
    for p in sorted(glob.glob(os.path.join(prev_dir, f"{stem}*.json"))):
        merged.update(json.load(open(p, "rt")))
    return merged
...
def main():
    ...
    dump(info, "info-indexed.json")
    dump(refs_indexed, "refs-indexed.json")
    dump(closures, "closures.json")
