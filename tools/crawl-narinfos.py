...
State is one file, --graph (jsonl), one record per crawled digest:
...
finish (one JSON record per line), so a run resumes
...
def append_graph(path, records):
    # Append each completed wave as plain JSON lines. Readers ignore malformed
    # trailing records if a previous writer was interrupted.
    #
